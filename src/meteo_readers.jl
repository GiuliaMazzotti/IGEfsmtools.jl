...
function read_meteo!(met::MET{Tf,Ti}, fsm::FSM{Tf,Ti}, t::DateTime, settings::Dict, ::Val{:GLAMOS_nc}) where {Tf<:Real,Ti<:Integer}

  # Prepare forcing data
  folder_met = joinpath(settings["met_folder"], Dates.format(t, "yyyy.mm"))
  filename_met = searchdir(folder_met, settings["met_prefix"] * "DATA_" * Dates.format(t, "yyyymmddHHMM"))
  ds = NCDataset(joinpath(folder_met, filename_met[1]))

  Sdir = ds["sdrd"][:,:]     # "direct shortwave radiation, per inclined surface area, within topography, above canopy"
  Sdif = ds["sdfd"][:,:]     # "diffuse shortwave radiation, per horizontal surface area, within topography, above canopy"
  LW = ds["lwrc"][:,:]       # "longwave radiation, above topography"
  Ptot = ds["prcs"][:,:]     # "precipitation"
  Ta = ds["tais"][:,:]       # "air temperature"
  RH = ds["rhus"][:,:]       # "relative humidity"
  Ua = ds["wnss"][:,:]       # "wind speed"
  Ps = ds["pail"][:,:]       # "local air pressure"
